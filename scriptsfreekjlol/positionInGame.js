class InGamePosition {
  constructor(setting, level) {
    ...
    this.ufos = [];
    this.bombs = [];
    this.laserUsed = false;
    this.laserActive = false;
    this.laserEndTime = null;
  }
  ...
  entry(play) {
    ...
    this.bombSpeed = this.setting.bombSpeed + (presentLevel * 1); //Level1: 75 + (1*10) = 85, Level2: 75 + (2*10) = 95, ...
    // 3. Bomb dropping frequency 
    this.bombFrequency = this.setting.bombFrequency + (presentLevel * 0.01); //Level1: 0.05 + (1*0.05) = 0.1, Level2: 0.05 + (2*0.05) = 0.15 ...


    this.spaceshipSpeed = this.setting.spaceshipSpeed;

    this.object = new Objects();
    this.spaceship = this.object.spaceship((play.width / 2), play.playBoundaries.bottom, play);
  ...
  draw(play) {
    ...
    ctx.drawImage(this.spaceship.getCurrentCostume(), this.spaceship.x - this.spaceship.width / 2, this.spaceship.y - this.spaceship.height / 2, this.spaceship.width, this.spaceship.height);
    ...
    if (this.laserActive) {
      ...
      ctx.fillRect(this.spaceship.x - 2, 0, 4, play.height);
    }
    ...
    ctx.fillText(
      "Press G to use Gaster Blaster (only once per level).",
  ...
  useLaser() {
    ...
    this.laserUsed = true;
    this.laserEndTime = new Date().getTime() + 500;
